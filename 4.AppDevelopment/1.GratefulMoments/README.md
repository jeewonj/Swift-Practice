...
2. **`import PhotosUI`, `photoPicker` 만들기** : 디바이스의 포토 라이브러리에 접근해서 사진을 고르는 UI.
    ...
    * `import PhotosUI`, `photoPicker` 만들기
    ...
    ```
    * `newImage` 변수 선언 + Image를 `PhotoPicker(selection:$newImage){}` 로 감싸기
    ```swift
    ...
    * `.onChange` Modifier 붙이기
    ```swift
        .onChange(of: newImage){
        guard let newImage else {return}
        }
    ```
    * `imageData` 변수 선언 + `Task` 붙이기
    ```swift
    ...
    ```
    * `if else` 문으로 이미지를 불러왔을 경우와 안불러왔을 경우 화면 구분하기 + `Group` 으로 아이콘과 photoArea 구분하기
     ```swift
        PhotosPicker(selection: $newImage){
            Group{
